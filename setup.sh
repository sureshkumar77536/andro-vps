...
# If we are running via `curl | bash` the lib/ui.sh isn't local yet — fetch it.
...
# 0. Always pull a fresh copy of run.sh + setup.sh so the user has them locally.
...
# 3. SDK packages (system image, emulator, platform-tools)
...
# 6. Mobile-friendly viewport meta in vnc.html (safe to re-apply check first)
